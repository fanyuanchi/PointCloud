	...
	public:
		Parser(vector<double> &origin, vector<double> &range, int r_max){
            ...
            for(int dim = 0; dim < 3; ++dim){
                this->origin[dim] = origin[dim];
                this->range[dim] = range[dim];
            }
		}
		...
		unsigned long long get_morton_code(Point *p) const{
            ...
            for(int d = 0; d < 3; d++){
                offset[d] = p->cord[d] - this->origin[d];
                semi[d] = 0;
            }
        ...
        static int range_relation(const double* s1, const double* e1, Subscriber *s){
